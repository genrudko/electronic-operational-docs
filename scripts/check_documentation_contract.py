...
FIXTURE_PATH = (
    ROOT / "tests/process/fixtures/industrialization_execution_cases.json"
)
...
def load_module_activation_contract(root: Path = ROOT) -> dict[str, Any]:
    ...
    if text.count(BEGIN) != 1 or text.count(END) != 1:
        raise ValueError(
            "module activation contract markers must appear exactly once"
        )
...
def validate_module_activation_contract(
    ...
    for field, expected in {
        ...
        if manifest.get(field) != expected:
            errors.append(
                _error(f"manifest-{field}", expected, manifest.get(field))
            )
    ...
    for transition in direct_transitions:
        if transition not in forbidden:
            errors.append(
                _error(
                    "forbidden-reactivation-transition",
                    transition,
                    sorted(forbidden),
                )
            )
    ...
    if lifecycle.get("disable_is_delete") is not False:
        errors.append(
            _error(
                ...
                False,
                lifecycle.get("disable_is_delete"),
            )
        )
    if lifecycle.get("retire_is_delete") is not False:
        errors.append(
            _error(
                "retire-preserves-history",
                False,
                lifecycle.get("retire_is_delete"),
            )
        )
    ...
    for state in read_limited_states:
    ...
    read_only_history = matrix.get("READ_ONLY", {}).get("detail_history")
    if read_only_history != "ALLOW":
        errors.append(
            _error(
    ...
    for state in ("INACTIVE", "RETIRED", "CONFIGURED"):
        detail_history = matrix.get(state, {}).get("detail_history", "")
        if "ALLOW_RETAINED_HISTORY" not in str(detail_history):
            errors.append(
                _error(
    ...
    for field in (
        ...
        if history.get(field) is not False:
            errors.append(
                _error(f"history-{field}", False, history.get(field))
            )
...
def apply_module_mutation(
    ...
    if operation == "set":
        ...
        raise ValueError(
            f"unsupported module activation fixture mutation: {operation}"
        )
    return mutated


def validate_module_activation_fixtures(
    contract: dict[str, Any],
) -> list[str]:
...
def run_module_activation_gate() -> int:
    try:
        ...
        print(
            f"Module activation architecture contract: FAILED: {exc}",
            file=sys.stderr,
        )
        return 1
    ...
    if errors:
        print(
            "Module activation architecture contract: FAILED",
            file=sys.stderr,
        )
        for error in errors:
