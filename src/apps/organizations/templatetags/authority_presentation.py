...
_REASON_LABELS = {
    ...
    "EXTERNAL_ENGAGEMENT_REQUIRED": (
        "Для внешнего сотрудника не зарегистрирован допуск "
        "принимающей организации."
    ),
    "EXTERNAL_ENGAGEMENT_NOT_EFFECTIVE": (
        "Срок внешнего допуска не охватывает момент проверки."
    ),
    "EXTERNAL_SCOPE_MISMATCH": (
        "Внешний допуск не распространяется на выбранный объект или область."
    ),
    ...
    "SUBSTITUTION_SCOPE_MISMATCH": (
        "Замещение не распространяется на выбранный объект или область."
    ),
}
...
_SUBJECT_LABELS = {
    ...
    "DEMO-AUTH-VERIFY": (
        "Разрешение на переключения по основанию, требующему подтверждения"
    ),
...
_BASIS_LABELS = {
    ...
    "UNCONFIRMED-AUTHORITY": (
        "Проект приказа о предоставлении права на разрешение переключений"
    ),
...
_DECISION_EXPLANATIONS = {
    "ALLOW": (
        ...
        "с подтверждённым основанием и действующей областью."
    ),
    "DENY": "На момент действия обязательные условия полномочия не выполнены.",
    "VERIFY": (
        "Система не может подтвердить право автоматически: требуется проверить "
        "документ-основание или исходные сведения."
    ),
}
...
def authority_decision_explanation(value: object) -> str:
    return _DECISION_EXPLANATIONS.get(
        str(value or "").upper(),
        "Результат сформирован по сведениям, действовавшим в момент проверки.",
    )
