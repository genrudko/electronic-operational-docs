...
## Repair 4 — click-away creation viewport stability

Ручная видеопроверка выявила отдельную гонку: при создании записи и щелчке
за пределами бумажного полотна асинхронная материализация завершалась уже после
потери фокуса, а отложенная пагинация перестраивала DOM после восстановления
viewport. В результате журнал перескакивал к началу смены.

Принятые решения:

- viewport новой строки фиксируется на capture-фазе `pointerdown`, до `blur`;
- намерение завершить создание сохраняется даже при уже выполняющемся fetch;
- click-away материализация проходит через единый `finishDraftEditing`;
- форма со статусом `finishing` считается активной для блокировки фоновой
  пагинации;
- ожидающая пагинация выполняется синхронно до восстановления anchor;
- отмена пустой строки также сохраняет положение страницы.

Cache revision: `011364`.
