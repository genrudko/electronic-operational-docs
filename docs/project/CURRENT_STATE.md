# ЭОД — текущее состояние

**Дата проверки:** 24.07.2026

**Принятый baseline:** `main / abd6066885b060e3e3d2c39098fcaf640bb70416`

**Рабочая ветка DOCS-001:** `docs/001-project-operating-system`

## 1. Статус проекта

Проект является независимым демонстрационным прототипом электронной оперативной документации для энергетики. Инициатива не является официальным поручением работодателя. Производственные серверы, фактические оперативные записи и реальные персональные данные в разработке не используются.

Целевой текущий рубеж — внутренний рабочий прототип всей оперативной документации; следующий рубеж — полностью функциональная демонстрационная версия. После DOCS-001 утверждённый план подлежит отдельной сверке с фактической реализацией и может быть скорректирован.

Базовый функциональный, инфраструктурный и процессный скелет проекта уже существует. Дальнейший основной фокус возвращается к последовательным продуктовым vertical slices; инфраструктурные расширения выполняются только по подтверждённой необходимости.

## 2. Принятый Git baseline

```text
branch: main
HEAD: abd6066885b060e3e3d2c39098fcaf640bb70416
included: INFRA-001 + INFRA-002 + INFRA-003
```

PR #3 `Infra 003: Isolated VPS development contour` принят пользователем и merged 24.07.2026.

## 3. Инфраструктура

### Accepted preview

```text
checkout: /srv/eod/repository
branch: main only
compose project: eod-preview
application: 127.0.0.1:8765
database: eod_preview
secrets: /srv/eod/secrets/preview.env
```

### Active development

```text
checkout: /srv/eod/development
branch: docs/001-project-operating-system
compose project: eod-development
application: 127.0.0.1:8766
database/user: eod_development
secrets: /srv/eod/secrets/development.env
```

Оба приложения и обе PostgreSQL-базы подтверждены одновременно healthy. PostgreSQL host ports не публикуются. Доступ к приложениям выполняется через SSH tunnel.

## 4. Данные

Accepted preview содержит перенесённый presentation profile:

- 8303 fixture objects;
- 84 Django models;
- две демонстрационные учётные записи;
- вымышленные персональные данные и безопасные презентационные справочники.

Development-база успешно создаётся как отдельная копия accepted preview, после чего на неё применяются миграции активной ветки.

## 5. Реализованные функциональные области

### Подтверждено реализовано

- Django application foundation;
- организация, подразделения, должности, сотрудники, учётные записи, роли и замещения;
- документарное ядро: черновики, версии, регистрация, нумерация, связи и аудит;
- повторная аутентификация, канонический snapshot и SHA-256 integrity status;
- нормативный реестр и редакции организационной конфигурации;
- энергообъекты, оборудование, диспетчерские наименования и их история;
- диспетчерское и технологическое управление и ведение;
- импорт оборудования и объектов диспетчеризации;
- импорт персонала, оперативных прав и документации рабочего места;
- специализированный оперативный журнал и сменная работа в объёме текущего прототипа;
- общее ядро структурированных журналов;
- source-bound каталог рабочих форм;
- GitHub Actions CI на Linux/PostgreSQL;
- безопасный preview и изолированный development на VPS.

### Реализовано частично или требует предметной приёмки

- формы конкретных структурированных журналов;
- связи между журналами и оперативным журналом;
- жизненные циклы заявок, распоряжений, дефектов, нарядов и переключений;
- шаблоны, сокращения и контекстная помощь оперативного журнала;
- печатные формы, экспорт и архивные представления;
- роли и переходы состояний для полного демонстрационного контура;
- возможный электронный справочный/контрольный контур журнала ключей.

### Не подтверждено как завершённое

- полный Structured Journals Pack;
- полноценный реестр нарядов и распоряжений;
- полный lifecycle допуска и работ;
- полный контур документов переключений;
- автоматическая генерация БП/ТБП/ТПП;
- юридически значимая электронная подпись;
- промышленная эксплуатационная готовность.

Полный электронный lifecycle выдачи и возврата ключей не входит в обязательный внутренний прототип: основной режим журнала ключей считается paper-first до отдельного предметного и UX-решения.

## 6. Текущий процесс разработки

Нормальный процесс больше не использует скачиваемые Python patch-файлы:

1. ассистент создаёт изменение в отдельной ветке GitHub;
2. CI проверяет изменение;
3. VPS development получает ветку через `git pull --ff-only`;
4. выполняются refresh/check/test/status;
5. пользователь проводит предметную и визуальную приёмку;
6. merge разрешается только пользователем;
7. preview синхронизируется после merge;
8. применимые canonical docs актуализируются вместе с изменением или обязательным post-merge follow-up.

Пользователь исключён из механической части программирования, но сохраняет постановку задачи и приёмку.

## 7. Текущий этап

`DOCS-001 — Project operating system`:

- создать единое дерево документации;
- заменить устаревший README;
- мигрировать `docs/project_state/`;
- закрепить GitHub-first/VPS-first workflow;
- добавить PR template и documentation CI gate;
- подготовить надёжный handoff;
- зафиксировать последовательную журнальную стратегию;
- исключить полный электронный lifecycle журнала ключей из обязательного prototype scope;
- запустить параллельный UX-001 с единым design and interaction contract.

## 8. Согласованное продуктовое направление

После PLAN-001 журнальный контур развивается по схеме:

```text
минимальный общий контракт
→ один журнал полностью
→ минимальные реальные связи
→ automated and user acceptance
→ следующий журнал
```

Связи не откладываются до завершения всего пакета. При этом полная универсальная timeline не проектируется заранее без подтверждённых реальных отношений.

Предварительный первый кандидат — журнал дефектов; окончательное решение принимается после evidence audit.

UX-001 развивается параллельно, проверяется на реальных экранах и не становится отдельным интеграционным центром.

## 9. Следующий этап

После принятия DOCS-001 проводится отдельная ревизия плана:

- сопоставить утверждённые Patch 011.5–011.16 с фактическим кодом;
- определить завершённые, частичные, изменённые и неактуальные задачи;
- проверить архитектурное направление;
- утвердить новый ближайший vertical slice и критерии внутреннего прототипа;
- подтвердить порядок последовательной журнальной разработки;
- определить минимальный автоматический test suite, поскольку текущая Django test command обнаруживает `0 test(s)`.

До этой ревизии `MASTER_PLAN.md` является базой анализа, а не автоматическим приказом реализовывать следующий исторический номер патча.
