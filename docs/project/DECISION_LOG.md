...
## 2026-08-03 — граница «черновик → чистовик» ОЖ зафиксирована

`OperationalDraftEntry` остаётся редактируемой рабочей строкой открытой смены с autosave, revisions и conflict detection. Изменение черновой строки не является исправлением зарегистрированной записи.

`OPJ-LIFECYCLE-001` владеет регистрацией отдельной подготовленной строки в неизменяемый `OperationalLogEntry` чистовика. После регистрации исходная строка сохраняет прослеживаемую связь, получает явное зарегистрированное состояние и больше не редактируется как черновик. Регистрация строки не закрывает смену и не заменяет передачу смены.

Исправление и отмена выполняются только в зарегистрированном журнале новой связанной записью или append-only событием с сохранением первоначального текста. Отдельный полноэкранный центр lifecycle и вывод digest/authority internals как основного интерфейса не требуются.

## 2026-08-03 — оперативные переговоры остаются частью хронологии ОЖ

В Demo вручную фиксируется оперативно значимый результат переговоров: команда, разрешение, подтверждение, отказ, сообщение об исполнении или о нарушении режима. Полная ручная карточка звонка и стенограмма не являются штатным сценарием.

Реквизиты участника, направления и канала вводятся компактно только при предметной необходимости. Автоматическая привязка аудиозаписей и метаданных специализированной системы регистрации переговоров относится к последующей интеграции.

## 2026-08-03 — ownership горизонтальных и вертикальных связей

`CROSS-DOC-001` владеет общесистемными typed relations, provenance, context snapshots, backlinks, source trail и relation graph.

Горизонтальные связи соединяют самостоятельные факты разных модулей одного процесса. Вертикальные связи обеспечивают причинно-временную прослеживаемость от основания через исполнение к результату, исправлению, отмене или закрытию.

До `CROSS-DOC-001` каждый модуль может реализовать только узкую внутреннюю связь собственного lifecycle. Такая связь не должна становиться параллельным универсальным relation engine, менять legal mode, предоставлять полномочия или дублировать первичный факт.
