...
- запуск: 23.07.2026 23:48:52 +03:00;
- исходный HEAD: `b73510a5b64b4f7faf9d80996c8ad3dba4822d6f`;
- Ruff, compileall, все static gate и Django checks прошли;
- профильные тесты Patch 011.7: 10/10;
- тесты импортера 011.6.2: 21/21;
- targeted-набор: 173/173, один skipped;
- полный последовательный suite: 495/495, один skipped;
- миграция `operational_documents.0001_initial` применена к presentation и development;
- диагностика обеих БД и collectstatic прошли;
- создан commit `fec8bd675f9565b0c4e398124cd22f8fabec02b4`;
- worktree clean, push не выполнялся;
- технический результат: успешно;
- визуальная проверка выявила неверную архитектурную границу и неудобный UX.

## Patch 011.7 Repair 2

- исходный HEAD: `fec8bd675f9565b0c4e398124cd22f8fabec02b4`;
- основание: визуальная проверка создания типа, записи, переходов, связей и поиска;
- ключевое решение: пользователь не конструирует журналы, формы устанавливаются только по утверждённым инструкциям;
- ручной веб-конструктор отключается;
- новые действия по техническим тестовым схемам блокируются;
- добавляется каталог source-bound форм из `И-00-007-ОР-2025 версия 2`, разделов 7–11 и приложений № 4–8;
- системные коды, SHA-256 и внутренний аудит переносятся в технический слой;
- форма записи делится на рабочие разделы, множественный выбор получает поиск и обычный щелчок;
- обязательный комментарий перехода получает явное сообщение;
- пустые дополнительные строки formset игнорируются даже при отправленном браузером значении первого `field_type`;
- технический результат определяется patch-логом;
- после успеха требуется короткая повторная визуальная приёмка.

## Patch 011.7 Repair 2 — первая попытка

- запуск: 24.07.2026 01:21:40 +03:00;
- исходный HEAD: `fec8bd675f9565b0c4e398124cd22f8fabec02b4`;
- лог: `patch_011_7_repair2_source_bound_journal_ux_20260724_012140.log`;
- snapshot, baseline, payload, backup и ранний `git diff --check` прошли;
- отказ произошёл на общем Ruff до Django checks, миграций и тестов;
- выявлены две E501 в `scripts/gate_patch_011_7.py` и один F401 в профильном тесте;
- обе runtime DB восстановлены из проверенных backup;
- новый `journal_forms.py` удалён, все изменённые файлы откатаны;
- worktree после rollback clean, commit не создавался;
- Revision 1 исправляет три Ruff-дефекта и проверяет AST, длину строк Python payload
  и отсутствие подтверждённого неиспользуемого импорта ещё в preflight до backup.
